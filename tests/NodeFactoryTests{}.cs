...
namespace tests
{
    public class NodeFactoryTests_Generic
    {
        ...
        public void NodeGraphFactory_CreateConnectedParallel_Validate(){
            ...
            var nodes = NodeGraphFactory.CreateConnectedParallel<Node<object>,object>(nodes_count,childs_count);
            validateConnected(nodes.Select(n=>n as NodeBase<object>).ToList(),nodes_count,childs_count);
        }
        [Fact]
        public void NodeGraphFactory_CreateRandomConnectedParallel_Validate(){
            ...
            const int nodes_count = 5000;
            var nodes = NodeGraphFactory.CreateRandomConnectedParallel<Node<object>,object>(nodes_count,max_childs_count, min_childs_count);
            validateRandomConnected(nodes.Select(n=>n as NodeBase<object>).ToList(),nodes_count,max_childs_count,min_childs_count);
        }
        ...
        public void NodeGraphFactory_CreateRandomConnected(){
            ...
            const int min_childs_count = 10;
            const int nodes_count = 5000;
            var nodes = NodeGraphFactory.CreateRandomConnected<Node<object>,object>(nodes_count,max_childs_count, min_childs_count);
            validateRandomConnected(nodes.Select(n=>n as NodeBase<object>).ToList(),nodes_count,max_childs_count,min_childs_count);
        }
        [Fact]
        public void NodeGraphFactory_CreateConnected_Validate(){
            const int childs_count = 100;
            const int nodes_count = 5000;
            var nodes = NodeGraphFactory.CreateConnected<Node<object>,object>(nodes_count,childs_count);
            validateConnected(nodes.Select(n=>n as NodeBase<object>).ToList(),nodes_count,childs_count);
        }
        private void validateRandomConnected<T>(IList<NodeBase<T>> nodes,int nodes_count,int max_childs_count, int min_childs_count){
            ...
            foreach(var node in nodes){
                ...
                var childs =new List<NodeBase<T>>(node.Childs.Select(n=>n.NodeBase));
                var hash_set = new HashSet<NodeBase<T>>(childs);
        ...
        private void validateConnected<T>(IList<NodeBase<T>> nodes,int nodes_count,int childs_count){
            Assert.Equal(nodes.Count,nodes_count);
            foreach(var node in nodes){
                //check if childs count of node equal to childs_count
