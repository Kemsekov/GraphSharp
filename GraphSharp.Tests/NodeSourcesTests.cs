...
namespace GraphSharp.Tests
{
    public class NodeSourcesTests
    {
        ...
        [Fact]
        public void SettingWrongNode_Throws()
        {
            foreach (var nodeSource in NodeSources)
            {
                Fill(nodeSource, 1000);
                for (int i = 0; i < 10; i++)
                {
                    var nId1 = Random.Shared.Next(1000);
                    var nId2 = nId1;
                    while(nId2==nId1)
                        nId2 = Random.Shared.Next(1000);
                    var n1 = nodeSource[nId1];
                    Assert.Throws<WrongNodeSetException>(() => nodeSource[nId2] = n1);
                }
            }
        }
        [Fact]
