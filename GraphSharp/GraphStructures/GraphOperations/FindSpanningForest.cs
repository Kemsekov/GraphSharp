...
where TEdge : IEdge
{
    ...
    KruskalForest<TEdge> FindForestDegree1(IGraph<TNode, TEdge> graph)
    {
        var edges = graph.Edges.OrderBy(x => x.Weight);
    ...
    public IList<TEdge> FindSpanningTreeDegree2OnNodes(Func<TEdge, float>? getWeight = null)
    {
        ...
        clone.Do.DelaunayTriangulation();
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            tmpGraph.Do.DelaunayTriangulation();
            foreach (var e in tmpGraph.Edges.ToList())
            {
            ...
            using var forest = FindForestDegree1(tmpGraph);
